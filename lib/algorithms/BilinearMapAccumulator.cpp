...
//    cout<<"Generating key...";
    ...
    future<void> pk2Future = threadPool.enqueue<void>(bind(computeG2Powers, cref(sk), q, ref(pk2)));
...
unique_ptr<G> computePower(const bool& inG2, const BilinearMapKey::PublicKey& publicKey,
    ...
    if(inG2){
        tempProduct = unique_new<G2DCLXVI>();
        ...
        tempProduct = unique_new<G1DCLXVI>();
...
//        cout << "Thread " << i << " will have offsets " << startOffset << " to " << endOffset << endl;
        ...
        workerFutures.push_back(
                threadPool.enqueue<unique_ptr<G>>(std::bind(computePower, ref(inG2), ref(publicKey),
                        ref(coeffs), startOffset, endOffset))
        );
    }
...
//    int i = -1;
    for(future<unique_ptr<G>>& workerFuture : workerFutures) {
//        i++;
...
void computeCoefficients(const vector<reference_wrapper<Scalar>>& roots, vector<unique_ptr<Scalar>>& coeffs){
    ...
    for(long i=0; i<degree; i++){
        unique_ptr<Scalar> pScalar = unique_new<ScalarDCLXVI>();
